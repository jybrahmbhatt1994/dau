...
import { TeachingFellowCard } from "@/components/teaching-fellows/TeachingFellowCard";
import { ChevronRight } from "@/components/ui/icons";
import type { TeachingFellowsTabData } from "@/lib/types";

const ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
const PAGE_SIZE = 10;

export function TeachingFellowsExplorer({
  tabs,
}: {
      ...
      <Container>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <input
            ...
            placeholder="Search Teaching Fellows..."
        ...
        {pageItems.length > 0 ? (
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {pageItems.map((fellow) => (
              <TeachingFellowCard key={fellow.id} fellow={fellow} />
            ))}
          </div>
        ) : (
          <p className="mt-10 text-center text-sm text-ash">
            No teaching fellows found.
          </p>
        )}
